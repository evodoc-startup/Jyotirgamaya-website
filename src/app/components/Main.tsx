    ...
    <motion.main
      ...
      <motion.div
        ...
        <div
          ...
          <p className="text-lg">{slides[slide].para}</p>
        </div>
      </motion.div>
      <div
        className={`absolute left-0 top-[40%] bg-${slides[slide].color} px-2 py-5 text-white cursor-pointer z-20`}
        onClick={() => setSlide(slide - 1 < 0 ? slides.length - 1 : slide - 1)}>
        <FaChevronLeft className="text-2xl" />
      </div>
      <div
        className={`absolute right-0 top-[40%] bg-${slides[slide].color} px-2 py-5 text-white cursor-pointer z-20`}
